...
window.DialogConnect = function(params) {
    ...
    !params && (params = {});
    !params.provider && (params.provider = 'univaultoffice');
    ...
    function _require_portal_info(portal, provider) {
        !provider && (provider = 'univaultoffice');
...
// NET_ERROR(CONNECTION_REFUSED, -102)
    ...
    return {
        show: function() {
            ...
            if ( _clouds.length == 1 && _clouds[0].provider == 'univaultoffice' ) {
                $combo.append(`<option value='univaultoffice'>univaultoffice</option>`);
                ...
                let $newportal = $el.find('.newportal').disable(!(params.provider=='univaultoffice'));
                $combo.on('change', e => {
                    $newportal.disable(!(e.target.value=='univaultoffice'));
                    _clear_error();
                });
